...
class GuideManagerComponent(MayaQWidgetDockableMixin, QtWidgets.QDialog):
    ...
    def create_window(self):
        ...
        self.setWindowTitle("Shifter 引导组件")
        self.resize(280, 600)
    ...
    def get_component_list(self):
        ...
        for path, comps in compDir.items():
            pm.progressWindow(
                title="正在加载组件", progress=0, max=len(comps)
            )
    ...
    def _component_menu(self, QPos):
        ...
        menu_item_01 = self.comp_menu.addAction("绘制组件")
        self.comp_menu.addSeparator()
        menu_item_02 = self.comp_menu.addAction("刷新列表")
    ...
    def _search_menu(self, QPos):
        ...
        menu_item_01 = self.search_menu.addAction("清除")
    ...
    def drag_draw_component(self, pGuide):
        if pGuide:
            ...
            pm.displayWarning("光标下方未捕捉到内容，未绘制组件")
