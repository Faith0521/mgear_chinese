...
class AutoFitBipedWidget(QtWidgets.QWidget):
    ...
    def _matchPositionToggled(self):
        ...
        if self.__matchPositionEnabled:
            self.__matchPositionEnabled = False
            self.enable_association_snap_cb.setText("启用位置匹配")

        else:
            self.__matchPositionEnabled = True
            self.enable_association_snap_cb.setText("禁用位置匹配")
    ...
    def embedSettingsInfoWidget(self):
        widget = QtWidgets.QGroupBox("嵌入节点设置")
        ...
        label_01 = QtWidgets.QLabel("嵌入分辨率")
        ...
        label_02 = QtWidgets.QLabel("嵌入类型")
        ...
        radio_label = QtWidgets.QLabel("镜像嵌入节点")
        ...
        self.mirror_embed_btns = QtWidgets.QPushButton("运行")
        ...
        smart_label = QtWidgets.QLabel("智能调整")
        ...
        self.smart_adjust_btn = QtWidgets.QPushButton("运行")
        ...
        self.match_guides_toEmbed_btn = QtWidgets.QPushButton("匹配引导")
        ...
        self.delete_embed_nodes = QtWidgets.QPushButton("删除嵌入节点")
    ...
    def exportEmbedInfoWidget(self):
        widget = QtWidgets.QGroupBox("创建/导出关联信息")
        layout = QtWidgets.QVBoxLayout()
        layout.setAlignment(QtCore.Qt.AlignTop)
        widget.setLayout(layout)
...
class RelativeGuidePlacementWidget(QtWidgets.QWidget):
    ...
    def ui(self):
        widget = QtWidgets.QGroupBox("相对引导放置设置")
        layout = QtWidgets.QVBoxLayout()
        layout.setAlignment(QtCore.Qt.AlignTop)
        widget.setLayout(layout)
        self.src_geo_widget = SelectComboBoxRefreshWidget("Source Mesh ")
        ...
        self.rgp_scale_cb = QtWidgets.QCheckBox("重置默认缩放")
...
class AutoFitGuideToolWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.relative_placement_widget = RelativeGuidePlacementWidget(
            parent=parent)
        self.afg_tab_widget.addTab(self.afb_widget, "自动适配双足")
        self.afg_tab_widget.addTab(self.relative_placement_widget,
                                   "相对引导放置")

    def loadSettingsWidget(self):
        self.load_settings_widget = QtWidgets.QGroupBox("加载模型 | 引导")
