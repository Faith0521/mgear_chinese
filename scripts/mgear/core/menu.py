...
def install_skinning_menu():
    ...
    commands = (
        ...
        ("蒙皮集群重命名", skin.rename_skin_clusters, "mgear_edit-3.svg"),
        ("-----", None),
        ...
        ("-----", None),
        ("获取 gSkin 文件中的名称", partial(skin.getObjsFromSkinFile, None)),
        ("-----", None),
        (
            "导入变形器权重贴图",
    ...
    mgear.menu.install("蒙皮与权重", commands, image="mgear_skin.svg")
...
def install_utils_menu(m):
    ...
    if versions.current() < 20220000:
        ...
        pm.menuItem(label="编译PyQt界面", command=pyqt.ui2py)
